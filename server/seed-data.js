// 种子数据模块 - 可被 server/index.js 动态导入用于自动初始化
...
const resources = [
  { title: '可爱小熊玩偶图解', type: 'pattern', category: 'doll', description: '超萌小熊玩偶详细图解，含头部、身体、四肢分片说明。使用棉线+3.0mm钩针，成品约15cm高。新手友好！', image_url: '/images/resources/resource-2.jpg', file_url: '#' },
  { title: '手提编织包图解（含内衬教程）', type: 'pattern', category: 'bag', description: '时尚手提包完整图解，含包体、提手、装饰花朵的钩法。额外附赠内衬缝制教程，实用与美观兼具。', image_url: '/images/resources/resource-4.jpg', file_url: '#' },
  { title: '经典菠萝花样方巾图解', type: 'pattern', category: 'scarf', description: '传统菠萝花样方巾图解，花样优雅细腻。使用蕾丝线钩织效果最佳，可作为桌布或披肩使用。', image_url: '/images/resources/resource-6.jpg', file_url: '#' },
  { title: '北欧风杯垫套装图解', type: 'pattern', category: 'other', description: '六款北欧几何风格杯垫图解，简约大方。使用棉线钩织，可作为家居装饰或伴手礼。', image_url: '/images/resources/resource-8.jpg', file_url: '#' },
  { title: '婴儿毯拼花图解合集', type: 'pattern', category: 'blanket', description: '5种花片拼接而成的婴儿毯图解，含花片钩法和拼接方法。柔软棉线材质，给宝宝最温柔的呵护。', image_url: '/images/resources/resource-10.jpg', file_url: '#' },
]
...
export function seedDatabase(db) {
  ...
  for (const c of comments) {
    insertComment.run(postIds[c.postIndex], c.content, c.author)
  }

  for (let i = 0; i < postIds.length; i++) {
    db.prepare('UPDATE posts SET likes = ? WHERE id = ?').run(postLikes[i], postIds[i])
  }

  console.log(`Seeded: ${yarnProducts.length} yarn + ${hookProducts.length} hook products, ${resources.length} resources, ${posts.length} posts`)
}
